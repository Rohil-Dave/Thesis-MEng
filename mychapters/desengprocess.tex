...
\begin{figure} [H] % opens the figure environment. the '[H]' forces the image to be Here
    ...
    \copyright {Birgitta Helmersson} % this prints the caption below the figure
\end{figure}
Her chart only considers the max chest/bust and max hip measurements to base sizing on. The fabric width is 31 cm greater than the largest max circumference given (here it is the hip). Given that the ease is 25 cm, the remaining 6 cm is the seam allowance around the width of the pattern. This 6 cm value is used in the project when scaling the pattern.
